...
const exportKitCodes = (kits: ScheduleKit[]) => {
  const codes = kits.map(k => k.codigo_kit).filter(Boolean).join('\n');
  const blob = new Blob([codes], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = 'codigos_cronograma.csv';
  a.click();
  URL.revokeObjectURL(url);
};

const ScheduleKitList = () => {
  // Data
  const [kits, setKits] = useState<ScheduleKit[]>([]);
  ...
  const [importFase, setImportFase] = useState('');
  ...
  const filteredKits = useMemo(() => {
    ...
    return kits.filter(kit => {
      ...
      const matchAct = !act ||
        kit.kit_actividades.some(a => a.codigo_actividad.toLowerCase().includes(act));
  ...
  // ── Activity form (inline inside modal) ──────────────────────────────────
  ...
  const alreadyImportedBaseIds = useMemo(() => {
    ...
    return new Set(currentKit.kit_actividades.map(a => a.base_actividad).filter(Boolean) as number[]);
  }, [currentKit]);

  const allFases = useMemo(() => {
    return Array.from(new Set(masterActivities.map(a => a.fase).filter(Boolean))).sort();
  }, [masterActivities]);

  const importDivisions = useMemo(() => {
    const seen = new Map<string, string>();
    masterActivities.forEach(a => {
      if (a.division_code && !seen.has(a.division_code))
        seen.set(a.division_code, a.division_name || a.division_code);
    });
  ...
  const filteredMaster = useMemo(() => {
    ...
    return masterActivities.filter(a => {
      ...
      const matchFase = !importFase || a.fase === importFase;
      const matchTerm = !term || a.codigo_actividad.toLowerCase().includes(term) || a.descripcion.toLowerCase().includes(term);
      return matchDiv && matchFase && matchTerm;
    });
  }, [masterActivities, importSearch, importFase, importDivision]);
  ...
  return (
    <div className="container py-4">
      <div className="d-flex justify-content-between align-items-start mb-3">
        <div>
          <h2 className="mb-0">Kits de Cronograma</h2>
          <p className="text-muted mb-0">Defina los kits de cronograma y sus actividades para gestionar el cronograma BIM</p>
        </div>
        <div className="d-flex gap-2">
          ...
          <button className="btn btn-primary shadow-sm" onClick={openNewKit}>
            <i className="bi bi-calendar3 me-2"></i>Nuevo Kit
          </button>
        </div>
      </div>
                  ...
                  {kit.kit_actividades.map(act => (
                    <div key={act.id} className="list-group-item d-flex justify-content-between align-items-center p-2">
                      <div className="small">
                        ...
                        {act.base_actividad && (
                          <span className="badge bg-light text-secondary border me-1" title="Derivada del catálogo">↑</span>
                        )}
                        <span className="text-truncate d-inline-block align-bottom" style={{ maxWidth: 210 }}>{act.descripcion}</span>
                      </div>
                      {act.division_code && (
                        <span className="badge bg-white text-muted border flex-shrink-0">{act.division_code}</span>
                      )}
                    </div>
                  ))}
            ...
            <div className="modal-content border-0 shadow-lg">
              <div className="modal-header bg-primary text-white py-3">
                <h5 className="modal-title fw-bold">
                  {currentKit ? `Editando Kit: ${currentKit.nombre}` : 'Nuevo Kit de Cronograma'}
                </h5>
                ...
                <div className="row g-0" style={{ minHeight: 500 }}>
                  ...
                  <div className="col-md-4 border-end bg-light p-4">
                    <form onSubmit={handleSaveKit}>
                      <h6 className="text-uppercase text-muted fw-bold small mb-3">Información del Kit</h6>
                      ...
                      <div className="mb-3">
                        ...
                        <input
                          ...
                          required
                          placeholder="Ej: CRON-001"
                          maxLength={50}
                        />
                        <div className="form-text">Código identificador del kit de cronograma.</div>
                      </div>
                      ...
                      <button type="submit" className="btn btn-primary w-100 fw-bold" disabled={savingKit}>
                        {savingKit
                          ...
                          : currentKit
                            ? <><i className="bi bi-check2 me-1"></i>Guardar Cambios</>
                            : <><i className="bi bi-plus-circle me-1"></i>Crear Kit</>
                        }
                      </button>
                      ...
                      {currentKit && (
                        <div className="mt-3 alert alert-info py-2 small mb-0">
                          ...
                          <strong>{currentKit.kit_actividades.length}</strong> actividades en este kit
                        </div>
                      )}
                    </form>
                  </div>
                      ...
                      <>
                        ...
                        <div className="border rounded">
                          <table className="table table-sm table-hover mb-0">
                            ...
                            <tbody>
                              {currentKit.kit_actividades.map(act => (
                                <tr key={act.id} className={editingActivityId === act.id ? 'table-warning' : ''}>
                                  ...
                                  <td className="align-middle">
                                    <small className="text-truncate d-block" style={{ maxWidth: 190 }}>{act.descripcion}</small>
                                    {(act.fase || act.sector) && (
                                      <small className="text-muted" style={{ fontSize: 10 }}>
                                        {[act.fase, act.sector].filter(Boolean).join(' · ')}
                                      </small>
                                    )}
                                  </td>
                                  <td className="align-middle">
                                    <small className="text-muted">{act.division_code || '—'}</small>
                                  </td>
                            ...
                            <div className="row g-2">
                              ...
                              <div className="col-md-3">
                                ...
                                <input
                                  type="text"
                                  className="form-control form-control-sm"
                                  value={activityForm.sector}
                                  placeholder="Ej: Sector A"
                                  onChange={e => setActivityForm(p => p && ({ ...p, sector: e.target.value }))}
                                />
                              </div>
                              <div className="col-md-6">
                                <label className="form-label small fw-semibold">División MasterFormat</label>
                                <select
                ...
                <div className="row g-2 mb-3">
                  <div className="col-5">
                    <div className="input-group input-group-sm">
                      ...
                      />
                      {importSearch && (
                        <button type="button" className="btn btn-outline-secondary" onClick={() => setImportSearch('')}>
                          <i className="bi bi-x"></i>
                        </button>
                      )}
                    </div>
                  ...
                  <div className="col-3">
                    <select className="form-select form-select-sm" value={importFase} onChange={e => setImportFase(e.target.value)}>
                      ...
                      {filteredMaster.map(act => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="align-middle"><small className="text-muted">{act.division_code || '—'}</small></td>
                            <td className="align-middle"><small className="text-muted">{act.fase || '—'}</small></td>
                          </tr>
                        );
                      })}
...
export default ScheduleKitList;
